...
export default function WorkerDashboard() {
  ...
  const updateStatus = async (bookingId, status) => {
    try {
      ...
      addToast(`Booking ${status}`); loadBookings();
    } catch (e) { addToast(e.message, "error"); }
  };
          ...
          <div className="dashboard-content">
            ...
            <div className="stats-row">
              {[
                ...
                { label: "Active",    value: accepted.length,  sub: "In progress" },
                { label: "Completed", value: completed.length, sub: "All time" },
                { label: "Rating",    value: user?.rating?.toFixed(1) || "—", sub: "★ Average" },
              ].map(s => (
            ...
            {loading ? <div className="empty-state"><span className="spinner" /></div>
              : pending.length === 0 ? (
                ...
                <BookingRow key={b._id} b={b}
                  onAccept={() => updateStatus(b._id, "accepted")}
                  onReject={() => updateStatus(b._id, "rejected")}
        ...
        {active === "bookings" && (
          <div className="dashboard-content">
            ...
            {loading ? <div className="empty-state"><span className="spinner" /></div>
              : bookings.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-icon">📅</div><p>No bookings yet</p>
                </div>
              ) : (
          ...
          <div className="dashboard-content">
            ...
            <div className="card" style={{ marginTop: 22 }}>
              ...
              {user?.document && (
                <>
                  <hr className="divider" />
                  <h4 style={{ fontFamily: "Playfair Display, serif", marginBottom: 14 }}>Documents</h4>
            ...
            <div className="card" style={{ marginTop: 22, maxWidth: 460 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <div className="form-group">
                  ...
                  <input type="password" className="form-input" value={pwForm.password}
                    onChange={e => setPwForm(f => ({ ...f, password: e.target.value }))} placeholder="••••••••" />
                </div>
