        ...
        {active === "home" && (
          <div className="dashboard-content">
            <div className="dash-header">
              <div>
                <h1 className="dash-title">Good day, <span className="accent-text">{user?.name?.split(" ")[0]}</span> 👋</h1>
            ...
            <div className="hero-search-bar">
              <span>🔍</span>
              <span style={{ color: "var(--text3)", fontSize: 14 }}>Search for a service...</span>
              <button className="btn btn-primary btn-sm" onClick={() => setActive("workers")}>Browse All</button>
            </div>

            <h3 className="section-title" style={{ marginBottom: 16 }}>Browse by Category</h3>
                  ...
                  <div key={w._id} className="worker-card card card-glow">
                    <div className="worker-card-top">
                      ...
                      <div>
                        ...
                        <div className="tag tag-provider" style={{ marginTop: 4 }}>{CAT_ICONS[w.serviceType]} {w.serviceType}</div>
                      </div>
                    ...
                    {w.isAvailable && (
                      <button className="btn btn-primary" style={{ width: "100%", marginTop: 12 }} onClick={() => setBookingModal(w)}>
                        Book Now
                      </button>
                    )}
                  </div>
                  ...
                  {bookings.map(b => (
                    <div key={b._id} className="booking-card card">
                      ...
                      {b.bookingStatus === "pending" && (
                        <button className="btn btn-danger btn-sm" style={{ marginTop: 12 }} onClick={() => cancelBooking(b._id)}>
                          Cancel Booking
                        </button>
                      )}
                    </div>
            ...
            <div className="profile-card card" style={{ marginTop: 20 }}>
              <div className="profile-header">
                ...
                <div>
                  ...
                  <div className="tag tag-client" style={{ marginTop: 6 }}>{user?.role}</div>
                </div>
              </div>
      ...
      {bookingModal && (
        <div className="modal-overlay" onClick={() => setBookingModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: "Syne, sans-serif", marginBottom: 8 }}>Book {bookingModal.name}</h3>
